...
/*                                                  +#+      +#+        +#+   */
/*   https://boj.kr/18809                          #+#        #+#      #+#    */
/*   Solved: 2026/03/09 19:22:41 by dolphine103   ###          ###   ##.kr    */
/*                                                                            */
/* ************************************************************************** */
namespace System;
class Program
{
    ...
    public static void Main()
    {
        ...
        int plantmany=Input(ref BoardHeight,ref BoardWidth,ref GreenStartCount,ref RedStartCount,ref Board,ref CanPlant);
        plant(BoardHeight,BoardWidth,GreenStartCount,RedStartCount,CanPlant,Board,0,plantmany);
        ...
        while(q.Count>0)
        {
            ...
            for(int i=0;i<4;i++)
            {
                ...
                if(board[Y][X]==board[a.Item1][a.Item2]) //같은 색으로 칠해짐
                    continue;
                if(board[Y][X]==1||board[Y][X]==2) //방문하지 않은 땅 타일
                {
                    board[Y][X]=board[a.Item1][a.Item2];
                    visit[Y][X]=visit[a.Item1][a.Item2]+1;
                    if(board[Y][X]==8)
                        countgreen+=1;
                    else if(board[Y][X]==9)
                        countred+=1;
                ...
                if(visit[Y][X]==visit[a.Item1][a.Item2]+1) //같은 시간에 서로다른 색이 만남
                {
                    ans+=1;
                    board[Y][X]=5;
                    continue;
    ...
    static int Input(ref int height,ref int width,ref int green,ref int red,ref int[][]board,ref (int,int)[]canplant)
    {
